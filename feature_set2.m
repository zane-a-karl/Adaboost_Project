...
function [Y, feat] = feature_set2(Imgs)
    ii = integralImage(Imgs); % this is 20x20*length(Imgs)
    ...
    c = 0;
    for px = 3:s - 8 % loops through possible pxs
        for py = 4:s-4 % loops through possible pys
            for w = 6:s-px % loops through possible widths
                for h = 3:s - py % loops through possible heights
